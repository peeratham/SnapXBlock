...
ReporterBlockMorph.prototype.toScriptXML = function (
    ...
    if (savePosition) {
        return serializer.format(
            '<script x="@" y="@">%'+decodeURI("%3C/script%3E"),
            position.x / scale,
            position.y / scale,
            this.toXML(serializer)
        );
    }
    return serializer.format('<script>%'+decodeURI("%3C/script%3E"), this.toXML(serializer));
};
...
CommandSlotMorph.prototype.toXML = function (serializer) {
    ...
    return '<script>'+decodeURI("%3C/script%3E");
};
